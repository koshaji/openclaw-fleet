...
# Ensure PATH includes common locations (needed when run from cron)
  ...
  for name in "${agents_to_update[@]}"; do
    ...
    if [[ "$healthy" == "true" ]]; then
      ...
      if [[ "$rb_healthy" == "true" ]]; then
        ...
        log_warn "Agent '$name' rolled back to previous image (healthy)"
      else
        registry_set_state "$name" "unhealthy"
        log_error "Agent '$name' rollback also unhealthy — manual intervention needed"
      fi
      update_failed=true
...
cmd_clone() {
  ...
  if [[ "$config_only" == "false" ]] && [[ -d "${source_dir}/workspace" ]]; then
    # Copy workspace (exclude session data and chat history)
    ...
    if command -v rsync &>/dev/null; then
      rsync -a --exclude='sessions/' --exclude='*.session' \
        "${source_dir}/workspace/" "${target_dir}/workspace/"
    else
      cp -a "${source_dir}/workspace" "${target_dir}/workspace"
      rm -rf "${target_dir}/workspace/sessions" 2>/dev/null || true
    fi
  else
    mkdir -p "${target_dir}/workspace"
  fi
...
cmd_shell() {
  ...
  if [[ ! "$name" =~ ^[a-zA-Z][a-zA-Z0-9_-]*$ ]]; then
    log_fatal "Invalid agent name '$name'"
  fi
  # Verify agent exists
  init_registry
  if [[ "$(registry_get_agent "$name")" == "null" ]]; then
    log_fatal "Agent '$name' not found in registry"
  fi
...
cmd_restore() {
  ...
  # Update only the gateway port/token in the restored config (preserve everything else)
  source_fleet_env

  # Patch openclaw.json with new port and token (keep all other settings from backup)
  ...
  if [[ -f "$config_file" ]]; then
    ...
    atomic_json_write "$config_file" "$patched"
  fi

  # Generate compose and .env with new ports (these are not user-edited)
  ...
  # Clean identity files (will be regenerated by the agent)
...
cmd_agentguard() {
  ...
  case "$action" in
    ...
    disable)
      grep -v '^AGENTGUARD_' "${FLEET_DIR}/.env.fleet" > "${FLEET_DIR}/.env.fleet.tmp" 2>/dev/null || true
      mv "${FLEET_DIR}/.env.fleet.tmp" "${FLEET_DIR}/.env.fleet"
      chmod 600 "${FLEET_DIR}/.env.fleet"
